...
export function OperationsPage({ api }: { readonly api: ConsoleApi }): React.JSX.Element {
  ...
  return <div className="page-stack"><header className="page-header"><div><p className="eyebrow">OPERATIONS</p><h1>任务与诊断</h1><p>这里只展示安全元数据，不显示 prompt 或凭证。</p></div></header>
    ...
    <section className="panel"><h2>后台任务</h2>{jobs.items.length === 0 ? <p className="muted">当前没有后台任务。</p> : <div className="job-list">{jobs.items.map((job) => <article key={job.jobId}><div><strong>{job.jobType}</strong><StatusBadge status={job.status} /></div><p>attempt {job.attempt}/{job.maxAttempts} · {Math.round(job.progress * 100)}%</p><small>{job.reasonCode}</small></article>)}</div>}</section>
