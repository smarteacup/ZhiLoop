...
export function HighRiskGovernancePanel({ api }: { readonly api: Pick<P4ConsoleApi, "highRiskGovernance" | "previewHighRisk" | "commitHighRisk"> }): React.JSX.Element {
  ...
  const previewFresh = preview !== undefined && preview.kind === kind && preview.policyRevision === state.value.policyRevision && Date.parse(preview.expiresAt) >= Date.now();
  ...
  const createPreview = async (): Promise<void> => {
    ...
    try { setPreview(await api.previewHighRisk({ expectedPolicyRevision: state.value.policyRevision, idempotencyKey: gate.idempotencyKey!, kind, assetIds: splitIds(assets), projectIds: splitIds(projects), reason: reason.trim(), payloadFingerprint: payloadFingerprint.trim() })); }
  ...
  const commit = async (): Promise<void> => {
    ...
    try { const receipt = await api.commitHighRisk({ previewId: preview.previewId, expectedPolicyRevision: preview.policyRevision, idempotencyKey: `p4.commit:${preview.previewId}`, confirmationPhrase: confirmation }); setMessage(`已提交 ${receipt.kind}：${receipt.operationId} · ${receipt.committedAt}`); setPreview(undefined); setConfirmation(""); }
