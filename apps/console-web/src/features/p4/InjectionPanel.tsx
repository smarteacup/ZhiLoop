...
function InjectionAttempt({ value }: { readonly value: InjectionAttemptView }): React.JSX.Element {
  ...
  return <article className="fact-row" aria-label={`Turn ${value.turnId} 注入 ${value.status}`}>
    ...
    <details><summary>知识指针与裁剪原因</summary><ul>{value.envelope.items.map((item) => <li key={`${item.knowledgeId}:${item.version}`}>{item.knowledgeId}@{item.version} · {item.detailLevel} · {item.estimatedTokens} tokens</li>)}</ul>{value.envelope.omitted.length === 0 ? <p className="muted">没有服务端报告的 omitted 项。</p> : <ul>{value.envelope.omitted.map((item) => <li key={`${item.knowledgeId}:${item.version}`}>{item.knowledgeId}@{item.version} · {item.reasonCode}</li>)}</ul>}</details>
