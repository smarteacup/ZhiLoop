...
export function SessionDetailPage({ api, sessionId }: { readonly api: ConsoleApi; readonly sessionId: string }): React.JSX.Element {
  ...
  return <div className="page-stack"><a className="back-link" href="#/sessions">← 返回会话</a><header className="page-header"><div><p className="eyebrow">{detail.summary.sessionId}</p><h1>{detail.summary.title}</h1><p>{detail.summary.projectHint ?? "未识别项目"} · 只读</p></div><StatusBadge status={detail.summary.captureStatus} /></header>
    ...
    {tab === "injections" ? <section className="panel" role="tabpanel"><div className="section-heading"><h2>注入记录</h2><span>SHADOW 与实际投递严格区分</span></div>{detail.injections.length === 0 ? <p className="muted">当前会话没有注入记录。</p> : <div className="injection-list">{detail.injections.map((attempt) => <article key={attempt.attemptId}><div><strong>{attempt.turnId}</strong><StatusBadge status={attempt.status} /></div><p>{attempt.knowledge.length} 条知识 · {attempt.estimatedTokens}/{attempt.maxTokens} tokens</p><small>{attempt.reasonCode}</small></article>)}</div>}</section> : undefined}
