...
import { SessionEvolutionTimeline } from "../evolution/SessionEvolutionTimeline.js";

export function SessionDetailPage({ api, sessionId }: { readonly api: ConsoleApi; readonly sessionId: string }): React.JSX.Element {
  const [tab, setTab] = useState<"timeline" | "chain" | "events" | "injections" | "extraction">("timeline");
  ...
  return <div className="page-stack"><a className="back-link" href="#/sessions">← 返回会话</a><header className="page-header"><div><p className="eyebrow">{detail.summary.sessionId}</p><h1>{detail.summary.title}</h1><p>{detail.summary.projectHint ?? "未识别项目"} · 只读</p></div><StatusBadge status={detail.summary.captureStatus} /></header>
    <CapturePanel api={api} sessionId={sessionId} sourceAvailable={detail.summary.sourceStatus === "AVAILABLE"} onCommitted={retry} onViewLedger={() => { setTab("events"); retry(); }} />
    <a className="secondary-button" href={`#/closure/${encodeURIComponent(sessionId)}`}>查看 P4 闭环与治理</a>
    <div className="tab-list" role="tablist" aria-label="会话详情"><button type="button" role="tab" aria-selected={tab === "timeline"} onClick={() => setTab("timeline")}>演进时间线</button><button type="button" role="tab" aria-selected={tab === "chain"} onClick={() => setTab("chain")}>生产链</button><button type="button" role="tab" aria-selected={tab === "extraction"} onClick={() => setTab("extraction")}>知识提取</button><button type="button" role="tab" aria-selected={tab === "events"} onClick={() => setTab("events")}>Ledger 内容</button><button type="button" role="tab" aria-selected={tab === "injections"} onClick={() => setTab("injections")}>注入记录</button></div>
    {tab === "timeline" ? <SessionEvolutionTimeline api={api} sessionId={sessionId} captureStatus={detail.summary.captureStatus} capturedAt={detail.summary.lastActivityAt} /> : undefined}
    ...
    {tab === "events" ? <section className="panel" role="tabpanel"><div className="section-heading"><div><h2>Ledger 已沉淀内容</h2><p className="muted">展示 Ledger 中已经脱敏的内容；不会修改原始 Codex 会话。</p></div><span>内容与事件 ID 可追溯</span></div>{events.items.length === 0 ? <p className="muted">当前会话没有已沉淀事件。</p> : <div className="event-list">{events.items.map((event) => <article key={event.eventId}><div><strong>#{event.sequence} · {event.eventType}</strong><time>{new Date(event.occurredAt).toLocaleString()}</time></div><dl className="detail-grid"><div><dt>事件 ID</dt><dd>{event.eventId}</dd></div><div><dt>关联 ID</dt><dd>{event.correlationId}</dd></div><div><dt>脱敏数量</dt><dd>{event.redactionCount}</dd></div><div><dt>内容状态</dt><dd>{event.payloadPurged ? "已按保留策略清除" : "脱敏内容可查看"}</dd></div></dl>{event.payloadPurged ? <p className="muted">该事件内容已清除，仅保留可追溯元数据。</p> : <><pre className="ledger-content-preview">{event.contentPreview ?? "（没有可展示的文本内容）"}</pre>{event.contentTruncated ? <p className="muted">单条内容已截断。</p> : undefined}</>}</article>)}</div>}<p className="muted">{events.nextCursor === undefined ? "已到末页" : "还有更多事件，可使用服务端游标继续读取"}</p></section> : undefined}
