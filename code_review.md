...
| 增/真实首屏时限 | 🔴 高 | `control-plane.ts`、`session-catalog/transcript-source.ts`、`console-gateway/runtime.ts` | 服务就绪后 Overview 仍同步刷新完整目录；结构化 sub-Agent 放大读取，且 HTTP/Unix 任一层短预算都会使首屏返回 503。 | P0 安装成功但真实 UI 首屏不可用。 | Overview 改为纯持久化快照查询；header 预检排除嵌套 sub-Agent、并发扫描 single-flight；显式会话查询在 HTTP/Unix 两层使用 15 秒冷盘有界预算。 |
...
| Coverage | statements 91.20%、branches 86.47%、functions 92.90%、lines 94.46% |
