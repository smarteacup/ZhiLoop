...
```bash
npm run build
npm run release:local -- --output /absolute/path/to/zhiloop-0.3.12
```
...
/absolute/artifact/apps/sidecar/dist/deploy-main.js \
  install --artifact /absolute/path/to/zhiloop-0.3.12 --json
```
...
```bash
/absolute/artifact/apps/sidecar/dist/deploy-main.js \
  install --artifact /absolute/path/to/zhiloop-0.3.12 \
