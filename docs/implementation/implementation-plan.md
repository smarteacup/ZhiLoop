...
**状态**：已通过  
**证据**：固定 100 Turn 数据集；5 次确认、95% 无人工、0/50 suppress 重复、0 次循环、0.10 次/Turn 自动续跑、0/20 门禁漏判。详见 `p6-gate-report.md`。
...
**依赖**：P5 Gate、CKL-701  
**状态**：已完成  
**交付物**：Codex 原生插件、仓库 Marketplace、可恢复安装/卸载、Hook 合并、sidecar 生命周期、健康检查和机器可读版本兼容声明。  
**验收条件**：
...
**依赖**：CKL-104、CKL-205、CKL-506、CKL-507、CKL-602、CKL-703  
**状态**：已完成  
**交付物**：唯一应用编排入口、组件生命周期、Hook/MCP 快路径、Worker 单飞、健康与 shutdown 协议。  
**验收条件**：

- 组件部分启动失败时逆序回滚，启动中停止不能重新进入 READY。
- Hook 失败开放，MCP 失败关闭，停止时取消在途请求。
