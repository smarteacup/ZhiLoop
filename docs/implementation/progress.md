...
| CKL-101 | 完成 | 待提交 | 四类 Codex Hook 到标准事件的适配、确定性 ID、脱敏 Fixture | 105 项累计模块测试；适配器约 61,547 events/s；1 高/3 中风险，全部修复 |
...
- 下一任务是 CKL-102：实现版本化 Transcript Adapter。它只增量解析对话记录并产生标准事件；持久化、后台消费和知识注入仍保持关闭，直至各自 Gate 通过。
