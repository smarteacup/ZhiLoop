...
ZhiLoop 不把符号位置、方法签名和调用链等可从代码重新计算的内容复制成长期知识正文。它主要保存业务边界、决策原因、兼容策略和实践经验，并用 CodeAnchor（代码查询指针）指向相关代码。CodeGraph 查询可沉淀为有界、带 Git/图 revision 的可复用产物；版本不匹配就失效并重新查询，产物不会成为第二份代码权威。知识采用两种互补的存储方式：

- **Markdown** 保存人能直接阅读和修改的知识正文；
- **SQLite 与检索索引** 保存元数据、范围、版本、状态、来源关系和召回所需索引。

### 借鉴腾讯 Agent Memory 的知识维护方式

[腾讯 Agent Memory](https://github.com/TencentCloud/TencentDB-Agent-Memory) 把内容分为 L0 对话、L1 原子记忆、L2 场景和 L3 长期画像：先保存来源，再逐层聚合；检索时先看场景等高层摘要，事实不足时再下钻到原子记忆和原始对话。ZhiLoop 沿用了“分层、可下钻、少量注入”的思路，但面向研发知识增加了更严格的维护规则：

- 新结论先与同项目、同分支、同场景的现有知识比较，再执行新增、更新、合并或跳过；边界冲突时保持分离或等待确认。
- 更新永远创建新版本，旧版本和来源会话继续保留，避免一次错误总结覆盖历史。
- L2 场景是可重建、可阅读的 Markdown 投影；SQLite 保存关系、状态和检索索引。
- 代码相关结论还要经过 Git revision、CodeGraph 和测试证据门禁；语义相似不能替代代码事实。
...
因此，目标架构不会再建设一套代码图谱，也不会把 CodeGraph 原始结果当作长期真相。新任务需要代码细节时，ZhiLoop 先检查同一项目和 revision 下是否已有可复用产物；不兼容时再实时取得当前代码事实：
...
这项 CodeGraph 联邦检索、知识保鲜、场景定位和有界产物复用能力已经接入生产组合；旧知识仍保持可读，但在缺少可信项目/分支/提交定位时，不会被当作当前代码事实自动注入。
...
我们重点观察四件事：重复背景是否减少、知识是否能在正确项目中召回、答案能否追溯到原始证据、后台运行是否不干扰 Codex。`0.5.1` 已通过 61 个脚本级 Gate、208 个 Vitest 测试文件（1,747 项）和全局覆盖率门槛，本机仍以 READY/SHADOW、Candidate Preview 优先的安全模式运行。
