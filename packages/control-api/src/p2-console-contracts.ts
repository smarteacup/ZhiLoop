...
const p2CandidateLocalizationViewSchema = z.strictObject({
  claimMode: z.enum(["CURRENT_STATE", "USER_DECISION", "FUTURE_REQUIREMENT"]),
  projectId: safeId,
  repositoryRemote: boundedText(4_096).min(1).optional(),
  observedBranch: boundedText(500).min(1).optional(),
  observedCommit: boundedText(64).regex(/^[0-9a-f]{7,64}$/u).optional(),
  dirty: z.boolean(),
  branchMode: z.enum(["EXACT_BRANCH", "BRANCH_LINEAGE", "ALL_BRANCHES"]),
  branchValue: boundedText(1_000).min(1),
  scenarioId: boundedText(1_020).min(1),
  scenarioKey: boundedText(500).min(1),
  scenarioTitle: boundedText(500).min(1),
  scenarioSummary: boundedText(4_000).min(1),
  taskIntents: z.array(boundedText(1_000).min(1)).max(100),
  entryPoints: z.array(boundedText(1_000).min(1)).max(100),
  applicability: z.array(boundedText(4_096).min(1)).max(100),
  nonApplicability: z.array(boundedText(4_096).min(1)).max(100),
  modulePaths: z.array(boundedText(1_000).min(1)).max(100),
  symbols: z.array(boundedText(1_000).min(1)).max(100),
});
...
export const p2ExtractionCandidateViewSchema = z.strictObject({
  ...
  localization: p2CandidateLocalizationViewSchema.optional(),
  ...
  evidenceChecks: z.array(z.strictObject({
    assertionId: safeId,
    kind: safeCode,
    status: z.enum(["SUPPORTED", "REFUTED", "UNKNOWN", "ERROR"]),
    reasonCodes: z.array(safeCode).max(100),
    evidenceId: safeId.optional(),
    codeGraphArtifact: z.strictObject({
      artifactId: safeId,
      operation: z.enum(["SYMBOL", "CALL_PATH", "IMPACT"]),
      status: z.enum(["ACTIVE", "SUSPECT"]),
      codeRevision: boundedText(500).min(1),
      graphRevision: boundedText(500).min(1).optional(),
      query: boundedText(2_000).min(1),
      factCount: z.number().int().nonnegative().max(50),
      bounded: z.boolean(),
      reasonCodes: z.array(safeCode).max(100),
    }).optional(),
  })).max(1_000),
...
export const p2KnowledgeDetailViewSchema = z.strictObject({
  ...
  localization: z.strictObject({
    state: z.enum(["COMPLETE", "LEGACY", "REVISION_UNRESOLVED", "INVALID"]),
    claimMode: z.enum(["CURRENT_STATE", "USER_DECISION", "FUTURE_REQUIREMENT"]).optional(),
    projectId: safeId.optional(),
    repositoryRemote: boundedText(4_096).min(1).optional(),
    observedBranch: boundedText(500).min(1).optional(),
    observedCommit: boundedText(64).regex(/^[0-9a-f]{7,64}$/u).optional(),
    dirty: z.boolean().optional(),
    branchMode: z.enum(["EXACT_BRANCH", "BRANCH_LINEAGE", "ALL_BRANCHES"]).optional(),
    branchValue: boundedText(1_000).min(1).optional(),
    scenarioId: boundedText(1_020).min(1).optional(),
    scenarioKey: boundedText(500).min(1).optional(),
    scenarioTitle: boundedText(500).min(1).optional(),
    scenarioSummary: boundedText(4_000).min(1).optional(),
    taskIntents: z.array(boundedText(1_000).min(1)).max(100),
    entryPoints: z.array(boundedText(1_000).min(1)).max(100),
    applicability: z.array(boundedText(4_096).min(1)).max(100),
    nonApplicability: z.array(boundedText(4_096).min(1)).max(100),
  ...
  scenario: z.strictObject({
    ...
    scenarioId: boundedText(1_020).min(1),
    version: positiveVersion.optional(),
    title: boundedText(500).min(1),
    summary: boundedText(4_000).min(1),
    knowledgeVersions: z.array(boundedText(1_000).min(1)).max(10_000),
    relationCount: z.number().int().nonnegative().max(1_000),
    markdown: boundedText(64_000).optional(),
  }).optional(),
  codeGraphArtifacts: z.array(z.strictObject({
    artifactId: safeId,
    operation: z.enum(["SYMBOL", "CALL_PATH", "IMPACT"]),
    status: z.enum(["ACTIVE", "SUSPECT"]),
    codeRevision: boundedText(500).min(1),
    ...
    factCount: z.number().int().nonnegative().max(50),
    bounded: z.boolean(),
    sourceRef: boundedText(4_000).min(1),
