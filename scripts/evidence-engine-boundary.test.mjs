...
test("CKL-303: Project Identity, seven Verifiers, Evidence traceability, and error isolation form one boundary", async () => {
  ...
  assert.deepEqual(results.map((result) => result.status), [
    "SUPPORTED", "SUPPORTED", "SUPPORTED", "SUPPORTED", "REFUTED", "SUPPORTED", "UNKNOWN", "UNKNOWN",
  ]);
  ...
  assert.equal(results[4].evidence?.verdict, "CONTRADICTS");
  assert.equal(results[6].evidence?.verdict, "INCONCLUSIVE");
  assert.deepEqual(results[7].reasonCodes, ["NO_VERIFIER_REGISTERED"]);

  const failed = await createMvpVerifierRegistry().verify(assertions[2], {
    ...context,
